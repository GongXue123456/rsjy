      ...
      <div v-if="this.active==0?true:false" class="threeModule">
        <div class="card1 certificates p15 f16 pr">
          ...
          :before-delete="handleDelete1" class="w" v-model="fileList1" :max-count="1"/>
              <div  class="mt5 mb20 p10 bgcfff f14 tc">营业执照</div>
           </div>
            <div>
             <van-uploader   name="uploader" camera :after-read="afterRead2"
          :before-delete="handleDelete2" class="w" v-model="fileList2" :max-count="1"/>
             <div  class="mt5 mb20 p10 bgcfff f14 tc">备案证</div>
           </div>
            <div>
             <van-uploader  name="uploader" camera :after-read="afterRead3"
          :before-delete="handleDelete3"  class="w" v-model="fileList3" :max-count="1"/>
             <div  class="mt5 mb20 p10 bgcfff f14 tc">经营者身份证</div>
           </div>
        </div>
      </div> 
  
      <div v-if="this.active==1?true:false" style="margin-bottom:60px;margin-top:100px;width:100%;flex-shrink: 0;
    box-sizing: border-box;">
        <div class="card1 p15 f16 pr">
          <p class="f18 mb15">请完善经营信息</p>
            <van-field v-model="form.record_code" label="备案证号" placeholder="请输入备案证号" input-align="right" @input="form.record_code=form.record_code.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入备案证号' }]" />
               <van-field  readonly clickable label="加工形式" type="textarea" rows="1" autosize v-model="options1_1[1]" placeholder="请选择加工形式" :rules="[{ required: true, message: '请选择加工形式' }]" input-align="right" right-icon="arrow" @click="showCheckbox(3)" />
               <van-field  readonly clickable label="加工类别" type="textarea" rows="1" autosize v-model="options2_1[1]" placeholder="请选择加工类别" :rules="[{ required: true, message: '请选择加工类别' }]" input-align="right" right-icon="arrow" @click="showCheckbox(4)"/>
               ...
               <van-field  v-model="form.place_area"  label="营业面积(㎡)" type="number" placeholder="请输入营业面积" input-align="right" @input="form.place_area=form.place_area.replace(/[, ]/g,'')" :rules="[{required: true,message: '请输入营业面积'},{ pattern: /^(^[1-9](\d+)?(\.\d{1,2})?$)|(^0\.0[1-9]{1}$)|(^0\.[1-9]{1}\d{0,1}$)/, message: '只允许输入大于0的数' }]"/>
           ...
           <div class="cardInfo power bgcfff p15 f16">
            <p class="f16">人员信息</p>
             <div class="bgf5 mt15" v-for="(item, index) in formList" :key="index">
              <van-field v-model="item.employ_name" name="姓名" label="姓名" placeholder="姓名" input-align="right" @input="item.employ_name=item.employ_name.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请填写姓名' }]" />
              ...
              <van-field name="uploader" label="健康证上传" input-align="right" :rules="[{ required: true, message: '请上传健康证' }]">
                <template #input> <van-uploader v-model="item.employ_health_url" camera  :max-count="1" :before-read='beforeRead(index)' :after-read="uploadPic"
           /> </template>
              </van-field>
       ...
       <div class="firstPage" v-if="this.active==2?true:false">
        <cube-scroll ref="scroll" :data="items">
          <div class="card p15 pr">
            <div class="cardInfo  f16">
              ...
              <p class="pt20"><span >食品加工</span></p>
              <p class="pt20"><span >四川省小作坊备案证</span></p>
         ...
         <div class="card1  p15 f16 pr">
          ...
          <van-field  v-model="form.operator"  label="经营者姓名" placeholder="请输入经营者姓名" input-align="right"  @input="form.operator=form.operator.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入经营者姓名' }]" />
        ...
        <getAddress v-if="showPicker" @closeTip="show1(5)"  @closeTip1='showAddress' :province='form.province'  :city='form.city' :area='form.area' :vill='form.vill' :town='form.town' :rules="[{ required: true, message: '请选择小作坊所在地' }]"/> 
      </div>
       <van-row gutter="20"  class="p15  footer">
          ...
          <van-col span="12" v-if="active==0||active==1"><van-button class="next cfff" round block type="primary" native-type="submit"> 下一步</van-button></van-col>
...
export default {
  components: {Nav,checkboxModel,getAddress},
  data() {
    return {
      active: 0,
      items: [],
  ...
  computed: {
    ...mapState({
      recordPage1: (state) => state.record.recordPage1
    })
  },
  created(){
  ...
  mounted(){
    ...
    var that=this;
      if(this.form.company_code){ this.showData()}else{
        that.form=Object.assign({}, that.recordPage1)
        this.form.companytag_code=this.$route.query.companytag_code
        this.form.business_form=this.$route.query.business_form
        this.options1_1=['',this.form.produce_form_list.join(',')] 
        this.options2_1=['',this.form.producetype_codes_list.join(',')]
  ...
  beforeRouteLeave(to,from,next){
    ...
    next(
      this.$store.dispatch('record/setNewPage', this.form)
    )
  },
  methods: {
    ...
    show2(index,val){
      if (val[1].substr(0,1)==','){val[1]=val[1].substr(1);}
        if(index==6){ this.options1_1=val
