      ...
      <van-form validate-first @submit="onSubmit" @failed="onFailed">
         ...
         <div v-if="this.active==0?true:false" class="threeModule">
          <div class="card1 certificates p15 f16 pr">
            ...
            <div>
              ...
              <div v-if="form.companytag_code==='1001574645421581111111105'" class="mt5 mb20 p10 bgcfff f14 tc" >食品经营许可证</div>
              <div v-else-if="form.companytag_code==='1001574645421581111111102'" class="mt5 mb20 p10 bgcfff f14 tc" >备案证</div>
            </div>
            <div>
              ...
              <div v-if="form.companytag_code==='1001574645421581111111105'"  class="mt5 mb20 p10 bgcfff f14 tc" >法人身份证</div>
              <div v-else-if="form.companytag_code==='1001574645421581111111102'" class="mt5 mb20 p10 bgcfff f14 tc" >经营者身份证</div>
            </div>
          </div>
        </div> 
        <div v-if="this.active==1?true:false" style="margin-bottom:60px;margin-top:100px">
          <div class="card1 p15 f16 pr">
            ...
            <van-field v-if="form.companytag_code==='1001574645421581111111102'" v-model="form.record_code" label="备案证号" placeholder="请输入备案证号" input-align="right"  @input="form.record_code=form.record_code.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入备案证号' }]" />
            <van-field v-if="form.companytag_code==='1001574645421581111111105'" v-model="form.credit_code" label="统一社会信用代码" placeholder="请输入统一社会信用代码" input-align="right" @input="form.credit_code=form.credit_code.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入统一社会信用代码' },{ max: 18, message: '请如实填写18位统一社会信用代码，以供核对'},{ pattern:/[^_IOZSVa-z\W]{2}\d{6}[^_IOZSVa-z\W]{10}/g,message: '请输入正确的统一社会信用代码'}]" />
            <van-field v-if="form.companytag_code==='1001574645421581111111105'" v-model="form.record_code" label="食品经营许可证号" placeholder="请输入食品经营许可证号" input-align="right" @input="form.record_code=form.record_code.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入食品经营许可证号' }]" />
            <van-field v-if="form.companytag_code==='1001574645421581111111102'" readonly clickable label="主营项目" type="textarea" rows="1"  :rules="[{ required: true, message: '请选择主营项目' }]" autosize v-model="options1_1[1]" placeholder="请选择主营项目" input-align="right" right-icon="arrow" @click="showCheckbox(3)" />
            <van-field v-if="form.companytag_code==='1001574645421581111111102' " readonly clickable label="兼营项目" type="textarea" rows="1" autosize :rules="[{ required: true, message: '请选择兼营项目' }]" v-model="options2_1[1]"  placeholder="请选择兼营项目" input-align="right" right-icon="arrow" @click="showCheckbox(4)" />
            <van-field v-if="form.companytag_code==='1001574645421581111111105'" readonly clickable label="经营范围" type="textarea" rows="1" autosize :rules="[{ required: true, message: '请选择经营范围' }]" v-model="options1_1[1]" placeholder="请选择经营范围" input-align="right" right-icon="arrow" @click="showCheckbox(3)" />
            ...
            <van-popup v-model="showdate1Picker" round position="bottom">
              <van-datetime-picker v-model="currentDate" type="date" title="选择年月日" :min-date="minDate" :max-date="maxDate" @confirm="onConfirm" @cancel="onCancel" /> </van-popup>
            <van-popup v-model="showdate2Picker" round position="bottom">
              <van-datetime-picker v-model="currentDate" type="date" title="选择年月日" :min-date="maxDate" :max-date="maxDate1" @confirm="onConfirm" @cancel="onCancel" /> </van-popup>
         ...
         <div class="firstPage" v-if="this.active==2?true:false">
          <cube-scroll ref="scroll" :data="items">
            <div class="card p15 pr">
              <div class="cardInfo f16">
                ...
                <p class="pt20"><span >食品流通</span></p>
                <p class="pt20"><span v-if="form.companytag_code==='1001574645421581111111102'">四川省小经营店备案证（流通）</span><span v-else-if="form.companytag_code==='1001574645421581111111105'">食品经营许可证（流通）</span></p>
          ...
          <div class="card1 p15 f16 pr">
            ...
            <van-field v-model="form.company_name" label="小作坊名称" placeholder="请输入小作坊名称" input-align="right" @input="form.company_name=form.company_name.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入小作坊名称' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入特殊符号' }]"  />
            <van-field v-if="form.companytag_code=='1001574645421581111111102'" v-model="form.operator" label="经营者姓名" placeholder="请输入经营者姓名" input-align="right" @input="form.operator=form.operator.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入经营者姓名' }]" />
            ...
            <van-field readonly clickable label="小作坊所在地" type="textarea" rows="1" autosize v-model="form.place_operate" placeholder="请选择小作坊所在地" input-align="right" right-icon="arrow" @click="show_Picker" :rules="[{ required: true, message: '请选择小作坊所在地' }]"  />
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        place_operate: "",
        address: "",
        produce_form_list: [], //加工形式
        producetype_codes_list: [], //加工类别
    ...
    this.init();
     var that=this;
      if(this.form.company_code){ this.showData()}else{
        that.form=Object.assign({}, that.recordPage2)
        this.form.business_form=this.$route.query.business_form
        this.form.companytag_code=this.$route.query.companytag_code
     this.form.company_code=this.$route.query.company_code
        if(this.form.companytag_code==='1001574645421581111111102'){ 
          this.options1_1=['',this.form.main_subject_list.join(',')] 
          this.options2_1=['',this.form.sub_subject_list.join(',')]
        }else if(this.form.companytag_code==='1001574645421581111111105'){
  ...
  beforeRouteLeave(to,from,next){
    ...
    next(this.$store.dispatch('record/setNewPage2', this.form))
  },
  methods: {
    ...
    submit() {
     ...
     cancel(){this.show=false;this.textarea='';},
    showCheckbox(val){
...
afterRead3 (file) {
    ...
    uploadPic2(file) {
      ...
      uploadImg(param).then(response => {
        loading.clear();
         if(this.form.companytag_code==='1001574645421581111111105'){ 
         ...
         }else if(this.form.companytag_code==='1001574645421581111111102'){
