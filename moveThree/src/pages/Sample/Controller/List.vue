    ...
    <div class="pt100 bgcfff">
       <div v-if="items.length">
          <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad"  >
               <div class="reviewed pr m15 p15 pt15 bgf5" v-for="(item,index) in items" :key="index" @click="$router.push('/sampleListController/tree?id='+item.company_code)">
          ...
          <div class="address  f15 mt15 view-text">{{item.province+item.city+item.area+item.town+item.vill}}
           ...
           <div class="address  f15 mt15 view-text"><span class="fl general_color">留样次数：{{item.food_sample_count}}</span><span class="fr">{{item.food_sample_time_last}}</span></div>
    ...
    return {
      items:[],
       params: {
        ...
        pager_openset: 10,
        company_name: "",
        query_param:'food_sample_countbig'
      },
      loading: false,
      finished: false,
    };
