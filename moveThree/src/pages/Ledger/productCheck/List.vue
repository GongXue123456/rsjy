 <template>
  <div class="recordManagement bgcfff">
    ...
    <div class="record w">
      ...
      <div class="ml15 mr15">
         <van-field class="bgcf0"  clickable label="搜索日期"  clearable v-model="params.report_time" placeholder="请选择搜索日期" input-align="right" right-icon="arrow" :rules="[{ required: true, message: '请选择日期' }]"  @click="showdate1Picker = true"/>
       </div>
    </div>
    <div class="scroll-list-wrap" :style="{ height: fullHeight + 'px' }">
      <cube-scroll ref="scroll" :data="items" :options="options" @pulling-up="onPullingUp">
        <div class="reviewed pr m15 p15 pt15 bgf5" v-for="(item,index) in items" :key="index" @click="toExamine(item)">
          ...
          <div class="address  f15 mt15 view-text">{{item.report_time}}
           </div>
           <span class="pa iconfont iconxiangyou"></span>
        </div>
        <empty v-show=flag title="暂无产品检验记录" />
      </cube-scroll>
    </div>
    ...
    <div class="footer p15 bgcfff w"> 
        <van-row  gutter="20">
          <van-col span="24"><van-button  block type="primary" native-type="submit" @click="Add"> 新增产品检验记录</van-button></van-col>
...
export default {
  ...
  methods: {
    getList(params) {
      axios.get("app/standingbook/productcheck/list", params).then(v => {
    ...
    Add(){
        this.$router.push({path: "/productCheck/add",query: {
