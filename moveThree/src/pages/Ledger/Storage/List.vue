 <template>
  <div class="recordManagement bgcfff">
    ...
    <div class="scroll-list-wrap" :style="{ height: fullHeight + 'px' }">
      <cube-scroll ref="scroll" :data="items" :options="options" @pulling-up="onPullingUp">
        <div class="reviewed pr m15 p15 pt15 bgf5" v-for="(item,index) in items" :key="index" @click="toExamine(item)">
          ...
          <div class="address  f15 mt15 view-text">{{item.storage_in_time}}
           </div>
           <span class="pa iconfont iconxiangyou"></span>
        </div>
        <empty v-show=flag title="暂无成品入库记录" />
      </cube-scroll>
    </div>
    ...
    <div class="footer p15 bgcfff w"> 
        <van-row  gutter="20">
          <van-col span="24"><van-button  block type="primary" native-type="submit" @click="Add"> 新增成品入库记录</van-button></van-col>
...
export default {
  ...
  methods: {
    getList(params) {
      axios.get("app/standingbook/storage/list", params).then(v => {
    ...
    Add(){
        this.$router.push({path: "/storage/add",query: {
