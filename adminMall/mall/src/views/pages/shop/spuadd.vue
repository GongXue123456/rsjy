...
export default {
  ...
  methods: {
    ...
    onSubmit (formName) {
      ...
      if (this.defaultSlideImg.length === 0) {
        this.$message({
          message: '请上传轮播图',
          type: 'warning'
        })
      ...
      if (this.defaultDetailImg.length === 0) {
        this.$message({
          message: '请上传详情图',
          type: 'warning'
        })
      ...
      this.form.sell_attribute = this.getNewAttr(this.sell)
      this.form.p_slide_img = JSON.stringify(this.defaultSlideImg)
      this.form.p_detail_img = JSON.stringify(this.defaultDetailImg)
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ajax.post('/product/save', this.form)
        }
      })
    },
    /**
    ...
    successSlideImg (response, file, fileList) {
      if (fileList.length > 3) {
        fileList.splice(0, 1)
        this.defaultSlideImg.splice(0, 1)
      }
    ...
    successDetaiImg (response, file, fileList) {
      if (fileList.length > 3) {
        fileList.splice(0, 1)
        this.defaultDetailImg.splice(0, 1)
      }
