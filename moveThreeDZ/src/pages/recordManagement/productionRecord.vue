  ...
  <div>
    ...
    box-sizing: border-box;">
        <div class="card1 p15 f16 pr">
          <p class="f18 mb15">请完善经营信息</p>
            <van-field v-model="form.record_code" label="备案证号" placeholder="请输入备案证号" input-align="right" @input="form.record_code=form.record_code.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入备案证号' }]" />
               <van-field  readonly clickable label="生产形式" type="textarea" rows="1" autosize v-model="options1_1[1]" placeholder="请选择生产形式" :rules="[{ required: true, message: '请选择生产形式' }]" input-align="right" right-icon="arrow" @click="showCheckbox(3)" />
               <van-field  readonly clickable label="生产类别" type="textarea" rows="1" autosize v-model="options2_1[1]" placeholder="请选择生产类别" :rules="[{ required: true, message: '请选择生产类别' }]" input-align="right" right-icon="arrow" @click="showCheckbox(4)"/>
       ...
       <div class="firstPage" v-if="this.active==2?true:false">
        ...
        <getAddress v-if="showPicker" @closeTip="show1(5)"  @closeTip1='showAddress' :province='form.province'  :city='form.city' :area='form.area' :vill='form.vill' :town='form.town' :rules="[{ required: true, message: '请选择企业所在地' }]"/> 
      </div>
