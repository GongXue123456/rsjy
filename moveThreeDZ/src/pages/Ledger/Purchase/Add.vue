    ...
    <div class="information bgcfff">
     <van-form validate-first @submit="onSubmit"  @failed="onFailed">  
        <div class="card1  p15 f16 pr">
          ...
          <van-field  v-model="form.batching_name"  label="原辅料名称" placeholder="请输入原辅料名称" input-align="right" @input="form.batching_name=form.batching_name.replace(/[, ]/g,'')" :rules="[{ required: true, message: '请输入辅料名称' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入特殊符号' }]" />
          ...
          <van-field  v-model="form.produce_unit"  label="原辅料生产单位" placeholder="请输入原辅料生产单位" input-align="right"   />
          <van-field  v-model="form.produce_licence"  label="生产许可证编号" placeholder="请输入生产许可证编号" input-align="right"  />
          <van-field v-model="form.produce_batch"  label="生产日期（批号）" placeholder="请输入生产日期（批号）" input-align="right" />
