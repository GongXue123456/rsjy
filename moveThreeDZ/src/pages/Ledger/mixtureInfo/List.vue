 <template>
  <div class="recordManagement bgcfff">
    <Nav title="投料记录" />
    <div class="record w">
      <search @myvalue="ser" :name.sync="params.info_name" type="home" />
      <div class="ml15 mr15">
         <van-field class="bgcf0"  clickable label="搜索日期"  clearable v-model="params.info_time" placeholder="请选择搜索日期" input-align="right" right-icon="arrow" :rules="[{ required: true, message: '请选择日期' }]"  @click="showdate1Picker = true"/>
       </div>
    </div>
    <div class="scroll-list-wrap" :style="{ height: fullHeight + 'px' }">
      <cube-scroll ref="scroll" :data="items" :options="options" @pulling-up="onPullingUp">
        <div class="reviewed pr m15 p15 pt15 bgf5" v-for="(item,index) in items" :key="index" @click="toExamine(item)">
          <h3 class="c000 f18 fb">{{item.info_name}}</h3>
          <div class="address  f15 mt15 view-text">{{item.info_time}}
           </div>
           <span class="pa iconfont iconxiangyou"></span>
        </div>
        <empty v-show=flag title="暂无投料记录" />
      </cube-scroll>
    </div>
    ...
    <div class="footer p15 bgcfff w"> 
        <van-row  gutter="20">
          <van-col span="24"><van-button  block type="primary" native-type="submit" @click="Add"> 新增投料记录</van-button></van-col>
...
export default {
  ...
  methods: {
    getList(params) {
      axios.get("app/standingbook/mixtureinfo/list", params).then(v => {
    ...
    Add(){
        this.$router.push({path: "/mixtureInfo/add",query: {
