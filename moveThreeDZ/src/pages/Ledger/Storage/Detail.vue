    ...
    <div class="information bgcfff">
     <van-form >  
        <div class="card1  p15 f16 pr">
          ...
          <van-field readonly v-model="form.storage_in_batch"  label="入库批次（生产日期）" input-align="right"/>
          <van-field readonly clickable label="入库日期"  v-model="form.storage_in_time" input-align="right"  />
          <van-field readonly v-model="form.storage_in_num"  label="入库数量" input-align="right"/>
          <van-field readonly v-model="form.storage_out_batch"  label="出库批次（生产日期）" input-align="right"/>
          <van-field readonly v-model="form.storage_out_time"  label="出库日期"  input-align="right"/>
          <van-field readonly v-model="form.storage_out_num"  label="出库数量" input-align="right" />
          <van-field readonly v-model="form.balance_num"  label="结存量" input-align="right"/>
      ...
      <div v-if="user_type=='食品经营者'" class="footer p15 bgcfff w"> 
        <van-row  gutter="20">
          <van-col span="12"><van-button   block type="primary" native-type="submit" @click="$router.push('/storage/add?id='+company_code+'&id1='+storage_code)"> 编辑 </van-button></van-col>
  ...
  data() {
   ...
   created() {
     ...
     this.company_code=this.$route.query.id;
      this.storage_code=this.$route.query.id1;
     this.getDate()
  },
  methods: {
    getDate(){
      axios.get('app/standingbook/storage/single', {storage_code:this.storage_code}).then(v => {
        this.form=v.data.storage
      })
    },
    onDelete(){
        this.$dialog.confirm({message: '确认删除？',}).then(() => {
         axios.delete('app/standingbook/storage/delete', {storage_code:this.storage_code}).then(() => {
        this.$router.back()
      })
     }).catch(() => {});  
    },
