    ...
    <div class="information bgcfff">
     <van-form >  
        <div class="card1  p15 f16 pr">
          <van-field readonly clickable label="销售日期"  v-model="form.sale_time" input-align="right"  />
          <van-field readonly v-model="form.product_name"  label="产品名称" input-align="right" />
          <van-field readonly v-model="form.product_spces"  label="规格" input-align="right"/>
          <van-field readonly v-model="form.sale_num"  label="出厂数量" input-align="right"/>
          <van-field readonly v-model="form.check_no"  label="出厂检验报告编号" input-align="right"/>
          <van-field readonly v-model="form.product_batch"  label="生产日期（批号）" input-align="right"/>
          <van-field readonly v-model="form.buy_area"  label="销往地区"  input-align="right"/>
          <van-field readonly v-model="form.buy_name"  label="产品接收单位" input-align="right" />
          <van-field readonly v-model="form.link_name"  label="接收单位联系人" input-align="right"/>
          <van-field readonly v-model="form.link_phone"  label="接收单位联系电话"  input-align="right"/>
          <van-field readonly v-model="form.sender"  label="发货人" input-align="right"/>
      ...
      <div v-if="user_type=='食品经营者'" class="footer p15 bgcfff w"> 
        <van-row  gutter="20">
          <van-col span="12"><van-button   block type="primary" native-type="submit" @click="$router.push('/sale/add?id='+company_code+'&id1='+sale_code)"> 编辑 </van-button></van-col>
  ...
  data() {
   ...
   created() {
     ...
     this.company_code=this.$route.query.id;
      this.sale_code=this.$route.query.id1;
     this.getDate()
  },
  methods: {
    getDate(){
      axios.get('app/standingbook/sale/single', {sale_code:this.sale_code}).then(v => {
        this.form=v.data.sale
      })
    },
    onDelete(){
        this.$dialog.confirm({message: '确认删除？',}).then(() => {
         axios.delete('app/standingbook/sale/delete', {sale_code:this.sale_code}).then(() => {
        this.$router.back()
      })
     }).catch(() => {});  
    },
