<template>
   <el-container class="patroladd">
    <el-header class="box add bb">
        <h5 v-if="id==1" class="mb10"><span class="lines"></span>备案通过</h5>
        <h5 v-if="id==2" class="mb10"><span class="lines"></span>备案驳回</h5>
        <h5 v-if="id==3" class="mb10"><span class="lines"></span>巡查不合格</h5>
        <h5 v-if="id==4" class="mb10"><span class="lines"></span>培训不合格</h5>
        <h5 v-if="id==5" class="mb10"><span class="lines"></span>自查自纠</h5>
        <h5 v-if="id==6" class="mb10"><span class="lines"></span>投诉举报待处理</h5>
        <h5 v-if="id==7" class="mb10"><span class="lines"></span>投诉举报已处理</h5>
        <h5 v-if="id==9" class="mb10"><span class="lines"></span>备案待审核</h5>
        <h5 v-if="id==10" class="mb10"><span class="lines"></span>待巡查工单</h5>
        <div>
    ...
    <el-main class="p15 pt0">
     ...
     <el-row class="box mt20  f14">
        ...
        <el-form-item class="w " label-width="14%"   label="食品监管接收人：" size="small" >
          ...
          <el-col :span="22" :offset="1" class="tl">
        <el-form-item class="w " label-width="14%"   label="小作坊用户接收人：" size="small" >
  ...
  watch:{
    $route:{
      handler(val,oldval){
         ...
         this.resetPage()
         this.formInline.mess_event=val.params.id;
          this.getdate1() 
      },
      deep: true
    }
    ...
    getdate2(){
      axios.get('/api/mess/single',{mess_event:this.id}).then((v) => {
        var ss=JSON.stringify(v)=='{}';
        if(!ss){
          this.formInline=v.mess;
          this.activeClass=this.formInline.mess_type-1;
          this.mess_receive_person=v.mess.mess_receive_person.indexOf(",") != -1?v.mess.mess_receive_person.split(','):v.mess.mess_receive_person.split(' ')
           this.cities=v.mess.companytype_codes.indexOf(",") != -1?v.mess.companytype_codes.split(','):v.mess.companytype_codes.split(' ')
            this.checkedCities=v.mess.companytag_codes.indexOf(",") != -1?v.mess.companytag_codes.split(','):v.mess.companytag_codes.split(' ')
    ...
    IsActive(k,val) {
      ...
      this.formInline = {
          ...
          submitForm(formInline){
            this.formInline.mess_event=this.id;
        ...
        if (valid) {
         axios.post('/api/mess/save', paramas).then((v) => {
      })
        }
         })
