<template>
  <el-container class="dispose">
    <el-header class="box add bb">
      <h5 class="mb10">
        <span class="lines"></span>处理方式管理
      </h5>
      <div>
        <el-button
          icon="el-icon-plus"
      ...
      <div class="box list">
        <el-table :data="tableData" style="width: 100%" border fit>
          ...
          <el-table-column
            ...
            label="处理方式名称"
          >
            <template slot-scope="scope">{{scope.row.dispose_name}}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" min-width="80" align="center" fixed="right">
            <template slot-scope="scope">
              <el-button
                ...
                class="orange disib"
                size="mini"
                @click="goedit(scope.row.dispose_code,1)"
              >编辑</el-button>
              <el-button
                type="text"
                class="disib"
...
export default {
  ...
  computed: {
    ...mapGetters({ disposePage: "dispose/disposePage" })
  },
  mounted() {
    this.page = Object.assign({}, this.disposePage);
    this.currentPage =
  ...
  methods: {
    init(params) {
      axios.get("/api/dispose/list", params).then(v => {
    ...
    goedit(index,index1){
     this.$router.push({name: "DisposeAdd",query: {id: index,id1:index1}});
    },
    changeState(row) {
      let { dispose_code, state } = row;
      state = state == 1 ? 2 : 1;
      axios
        .put("/api/dispose/state/update", { dispose_code, state })
        .then(response => {
          this.init(this.page);
        });
    },
  ...
  beforeRouteLeave(to, from, next) {
    if (to.name == "DisposeAdd") {
      this.$store.dispatch("dispose/setDisposePage", this.page);
      next();
    }
  },
};
