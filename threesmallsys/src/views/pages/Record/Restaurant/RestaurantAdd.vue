<template>
   <el-container class="recordadd ">
       <el-header class="box add bb">
        <h5 class="mb10"><span class="lines"></span>新增备案/食品餐饮</h5>
        <div>
          <el-button   v-if="menu_btns!=null&&menu_btns.indexOf('5-3-5')!== -1" class="" size="small" type="primary" @click="submitForm('formInline')">提交归档</el-button>
      ...
      <el-row class="box mt20 f14">
       ...
       <el-row class="box mt20  f14">
        ...
        <el-col :span="9" class="tl" :offset="1">
        <el-form-item   class="w" label-width="34%" label="小作坊名称：" size="small" prop="company_name" :rules="[{required: true,message: '请输入小作坊名称', trigger: 'blur' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入空格等特殊符号' }]">
         <el-input placeholder="请输入小作坊名称" maxlength="30" clearable autocomplete="off"  v-model="formInline.company_name" class="input-with-select">
            </el-input>
        </el-form-item>
        </el-col>
        <el-col  v-if="this.formInline.companytag_code=='1001574645421581111111104'" :span="9" class="tl" :offset="1">
        ...
        <el-form-item class="w" label-width="34%" label="法人姓名：" size="small"  prop="operator" :rules="[{required: true,message: '请输入法人姓名', trigger: 'blur' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入空格等特殊符号' }]">
         <el-input placeholder="请输入法人姓名" maxlength="10" clearable v-model="formInline.operator" class="input-with-select">
            </el-input>
        </el-form-item>
        </el-col>
         <el-col :span="9" class="tl" :offset="1">
         <el-form-item class="w" label-width="34%" label="联系电话：" size="small"  prop="mobilephone" :rules="[{required: true,message: '请输入手机号',trigger: 'blur'},{pattern: /^1[3|4|5|7|8|9][0-9]\d{8}$/,message: '手机号格式不对',trigger: 'blur'}]">
          <el-input placeholder="请输入11位手机号码" clearable v-model="formInline.mobilephone" class="input-with-select">
            </el-input>
        </el-form-item>
        </el-col>
         <el-col :span="9" class="tl" :offset="1">
         <el-form-item class="w" label-width="34%" label="身份证号：" size="small" :rules="[{required: true, message: '请输入身份证号', trigger: 'blur' },{ min: 15, max: 18, message: '请如实填写15或者18位号码，以供核对', trigger: 'blur' },{ pattern: /(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}$)/,message: '请输入正确的身份证号码',trigger: 'blur'}]">
      ...
      <el-row class="box mt20  f14">
         ...
         <el-col v-if="activeClass==0" :span="9" class="tl" :offset="1">
           <el-form-item class="w" label-width="34%" label="统一社会信用代码：" size="small" prop="credit_code" :rules="[{required: true, message: '请输入统一社会信用代码', trigger: 'blur' },{ max: 18, message: '请如实填写18位统一社会信用代码，以供核对', trigger: 'blur' }]" >
           <el-input placeholder="请输入统一社会信用代码"  clearable v-model="formInline.credit_code" class="input-with-select">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="9" class="tl" :offset="1" v-if="activeClass==0">
           <el-form-item class="w" label-width="34%" label="食品经营许可证号：" size="small"  prop="record_code" :rules="[{required: true, message: '请输入食品经营许可证号', trigger: 'blur' }]">
           <el-input placeholder="食品经营许可证号"  clearable v-model="formInline.record_code" class="input-with-select">
            </el-input>
          </el-form-item>
        </el-col>
         <el-col :span="9" class="tl" :offset="1" v-if="activeClass==1">
        ...
        <el-col :span="22" class="tl" :offset="1">
          ...
          <el-form-item  label="经营地址："  class="w" label-width="14%" size="small" prop="vill" :rules="[{required: true,message: '请选择经营地址', trigger: ['blur','change']}]">
        ...
        <el-form-item  label="详细地址：" class="w" label-width="34%" size="small" >
         ...
         <el-col :span="22" :offset="1" class="tl" v-if="activeClass==0">
         <el-form-item class="w w24" label-width="14%"   label="经营范围：" size="small" prop="business_range_list" :rules="[{required: true,message: '请选择经营范围',trigger: ['blur','change'] }]">
         ...
         <el-col :span="22" :offset="1" class="tl" v-if="activeClass==1">
         <el-form-item class="w w24" label-width="14%"   label="主营项目：" size="small" prop="main_subject_list" :rules="[{required: true,message: '请选择主营项目',trigger: ['blur','change'] }]">
         ...
         <el-col :span="9" class="tl" :offset="1" >
         <el-form-item class="w" label-width="34%" label="面积：" size="small" prop="place_area" :rules="[{required: true,message: '请输入面积', trigger: 'blur'},{ pattern: /^(^[1-9](\d+)?(\.\d{1,2})?$)|(^0\.0[1-9]{1}$)|(^0\.[1-9]{1}\d{0,1}$)/, message: '只允许输入大于0的数' }]">
    ...
    <el-button size="small" type="primary">点击上传</el-button>
      ...
      <el-table :data="formInline.companyEmployList" style="width: 100%" class="no-header">
        ...
        <el-table-column >
             <template slot-scope="scope">
           <el-image :src="user.qyurl+scope.row.employ_health_url"  :preview-src-list="[user.qyurl+scope.row.employ_health_url]"  fit="fill" style="width: 40px;height: 40px"></el-image>
             </template>
        </el-table-column>
  ...
  <div class="recordadd-upload">
              ...
              <el-dialog :visible.sync="dialogVisible">
               ...
               <div class="tc p15" v-if="formInline.business_form=='学校（含托幼机构）食堂'">法人证书</div>
              <div class="tc p15" v-else-if="formInline.business_form=='机关企事业单位食堂'||formInline.business_form=='工地食堂'||formInline.business_form=='其他食堂'">批准设立文件或证件</div>
              <div class="tc p15" v-else>营业执照</div>
              </div>
        <div class="recordadd-upload">
         <el-upload  
                ref="upload"
        ...
        <div class="tc p15" v-if="formInline.companytag_code=='1001574645421581111111104'">备案证</div>
           <div class="tc p15" v-else>食品经营许可证</div>
        </div>
       <div class="recordadd-upload">
         <el-upload 
                ref="upload"
              ...
              <el-dialog :visible.sync="dialogVisible">
               ...
               <div class="tc p15" v-if="this.formInline.companytag_code=='1001574645421581111111104'">经营者身份证</div>
<div class="tc p15" v-else>法人身份证</div>
        </div>
      </el-col>
     </el-row>
  ...
  data () {
    return {
     ...
     uploadDisabled:false,
      ...
      activeClass:0,
      list_btn:[{title: '食品经营许可（餐饮）',val:'1001574645421581111111103'},{title: '四川省小经营店备案证（餐饮）',val:'1001574645421581111111104'}],
      formInline: {
        company_code:'',
         companytype_code:'1001574645421581000000003',
        companytag_code:'1001574645421581111111103',
        company_name: '',
        credit_code:'',
  ...
  methods: {
    ...
    cancle(){ this.form.List=[{name: '',healthimg:'',time:''}]},
     savepeople(form){
      ...
      handleRemove(file, fileList) {
          ...
          if(this.formInline.business_form=='学校（含托幼机构）食堂'){
           this.$set(this.formInline.accessoryList, 0,{name:'法人证书',url:''});
         }else if(this.formInline.business_form=='机关企事业单位食堂'||this.formInline.business_form=='工地食堂'||this.formInline.business_form=='其他食堂'){
           this.$set(this.formInline.accessoryList, 0,{name:'批准设立文件或证件',url:''});
         }else{
      ...
      handleRemove1(file, fileList) {
          this.uploadDisabled1 = false;
           if(this.formInline.companytag_code=='1001574645421581111111104'){
           this.$set(this.formInline.accessoryList, 1,{name:'备案证',url:''});
         }else{
           this.$set(this.formInline.accessoryList, 1,{name:'食品经营许可证',url:''});
          }
      },
       handleRemove2(file, fileList) {
          this.uploadDisabled2 = false;
           ...
           this.$set(this.formInline.accessoryList, 2,{name:'法人身份证',url:''});
          }
      },
      ...
      handlepoll (response, file, fileList) {
         ...
         if(this.formInline.business_form=='学校（含托幼机构）食堂'){
            this.$set(this.formInline.accessoryList, 0,{name:'法人证书',url:response.data.visit_url.substring(response.data.visit_url.indexOf("resources"))});
         }else if(this.formInline.business_form=='机关企事业单位食堂'||this.formInline.business_form=='工地食堂'||this.formInline.business_form=='其他食堂'){
           this.$set(this.formInline.accessoryList, 0,{name:'批准设立文件或证件',url:response.data.visit_url.substring(response.data.visit_url.indexOf("resources"))});
         }else{
     ...
     handlepoll1 (response, file, fileList) {
         ...
         if(this.formInline.companytag_code=='1001574645421581111111104'){
           ...
           this.$set(this.formInline.accessoryList, 1,{name:'食品经营许可证',url:response.data.visit_url.substring(response.data.visit_url.indexOf("resources"))});
          }
    ...
    handlepoll2 (response, file, fileList) {
         this.uploadDisabled2 = true;
          ...
          this.$set(this.formInline.accessoryList, 2,{name:'法人身份证',url:response.data.visit_url.substring(response.data.visit_url.indexOf("resources"))});
         }
      ...
      handlePictureCardPreview(file) {
       ...
       IsActive(k,val) {
        ...
        this.formInline.business_form='';
        if(this.formInline.companytag_code=='1001574645421581111111104'){
           this.formInline.accessoryList=[{name:'营业执照',url:''},{name:'备案证',url:''},{name:'经营者身份证',url:''}]
        }else{
          this.formInline.accessoryList=[{name:'营业执照',url:''},{name:'食品经营许可证',url:''},{name:'法人身份证',url:''}]
        }
         this.init1()
      },
