<template>
  <el-container class="shopList">
    <el-header class="box add bb">
        <h5 class="mb10"><span class="lines"></span>乡厨管理 </h5>
        <div> <el-button size="small" type="primary" @click="godetail('','0','乡厨')">新增乡厨</el-button></div>
    </el-header>
    ...
    <div class="box list">
      <div class="tl w f20 p10 bgf5f c666">健康证过期：<span class="red mr20">{{User_health_over_count}}</span> 健康证正常：<span class="red">{{User_health_ok_count}}</span></div>
      ...
      <template scope="scope">
        ...
        <el-table-column label="操作" width="120" fixed="right"  align='center'>
          <template slot-scope="scope">
           <el-button type="text" class="orange" size="small" @click="godetail(scope.row.user_code,'1','乡厨')">编辑</el-button>
...
this.tableData=v.user_list
 ...
 this.User_health_ok_count=v.User_health_ok_count
      })
...
      if(this.page.endtime==null){this.page.endtime='';}
      if(this.page.addtime==null){this.page.addtime='';}
      this.page.down_name = '乡厨管理列表信息'
      axios.fileget('/api/report/export_chef_search', this.page).then((v) => {
      })
    },
   check(){
     axios.get('/api/report/check', {down_name:'export_chef_search'}).then((v) => {
       if(v==true){
