...
export default {
  data () {
    return {
      ...
      options2:[{name:'普通巡查',id:'1'},{name:'日常监督抽查',id:'2'},{name:'工单派发',id:'3'}],
      options4:'',
  ...
  methods: {
     ...
     userTypeList(row){if(row.patrol_type==1){return '普通巡查'}else if(row.patrol_type==2){return '日常监督抽查'}else if(row.patrol_type==3){return '工单派发'}},
          change(index,val){
             var that=this;
            if (val == null) val = '';
