<template>
  <el-container class="shopList">
    <el-main class="pt0 m10">
      <el-row  class="head">
       <el-col :span="6" class="w left f20 fb tl"><img class="ml20 mr10" src="../../assets/image/sanxiao-logo1.png">小作坊数智云平台</el-col>
      ...
      <el-row :gutter="40" class="main mt10 ml30 f14 mr30">
       <el-col :span="12" class="mb10"><div class="fr col pr"   ref="Purple" :style="{width:fullWidth/2 + 'px',height:fullWidth/2*0.50 + 'px'}">
         <span  @click="godetail1(play_video_name.company_area_code)" class="disib">{{play_video_name.company_area_name}}</span><i v-if="play_video_name!=''" @click="godetail1(play_video_name.company_area_code)" class="ml5 iconfont iconshexiangtou "></i> <el-button @click="handleChooseCompany(1)" type="primary" class="pa" size="mini">添加摄像头</el-button>
          <!-- <span  @click="godetail1(play_video_name.company_area_code)" class="disib">{{play_video_name.company_area_name}}</span> <el-button @click="handleChooseCompany(1)" type="primary" class="pa" size="mini">添加摄像头</el-button> -->
          <div>
             <video-player class="vjs-custom-skin" 
                        :options="playerOptions" 
                        @ready="playerReadied">
          </video-player>   
          <div class="video-text"><p class="mb5"><span class="tr">温度：</span><span class="tl">{{sensor_log1.temp}}℃</span></p><p class="mb5"><span class="tr">湿度：</span><span class="tl">{{sensor_log1.humidity}}</span></p><p class="mb5"><span class="tr">烟雾：</span><span class="tl">{{sensor_log1.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">甲烷：</span><span class="tl">{{sensor_log1.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">液化气：</span><span class="tl">{{sensor_log1.mq5_monitor_range}}</span></p><p class="mb5"><span class="tr">一氧化碳：</span><span class="tl">{{sensor_log1.mq9_monitor_range}}</span></p><p class="mb5"><span class="tr">水质：</span><span class="tl">{{sensor_log1.tds}}</span></p><p class="mb5"><span class="tr">酸碱度：</span><span class="tl">{{sensor_log1.ph}}</span></p></div>
          </div>
       </div>
       </el-col>
       <el-col :span="12" class="mb10"><div class="col pr" ref="Purple1" :style="{width:fullWidth/2 + 'px',height:fullWidth/2*0.50 + 'px'}"><span @click="godetail1(play_video_name1.company_area_code)">{{play_video_name1.company_area_name}}</span><i @click="godetail1(play_video_name1.company_area_code)" v-if="play_video_name1!=''" class="ml5 iconfont iconshexiangtou "></i> <el-button @click="handleChooseCompany(2)" type="primary" class="pa" size="mini">添加摄像头</el-button>
        <div>
              <video-player class=" vjs-custom-skin" 
                        :options="playerOptions1" 
                        @ready="playerReadied">
          </video-player>
            <div class="video-text"><p class="mb5"><span class="tr">温度：</span><span class="tl">{{sensor_log2.temp}}℃</span></p><p class="mb5"><span class="tr">湿度：</span><span class="tl">{{sensor_log2.humidity}}</span></p><p class="mb5"><span class="tr">烟雾：</span><span class="tl">{{sensor_log2.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">甲烷：</span><span class="tl">{{sensor_log2.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">液化气：</span><span class="tl">{{sensor_log2.mq5_monitor_range}}</span></p><p class="mb5"><span class="tr">一氧化碳：</span><span class="tl">{{sensor_log2.mq9_monitor_range}}</span></p><p class="mb5"><span class="tr">水质：</span><span class="tl">{{sensor_log2.tds}}</span></p><p class="mb5"><span class="tr">酸碱度：</span><span class="tl">{{sensor_log2.ph}}</span></p></div>
            </div></div></el-col>
       <el-col :span="12" class=""><div class="fr col pr" ref="Purple2" :style="{width:fullWidth/2 + 'px',height:fullWidth/2*0.50 + 'px'}"><span @click="godetail1(play_video_name2.company_area_code)">{{play_video_name2.company_area_name}}</span><i @click="godetail1(play_video_name2.company_area_code)" v-if="play_video_name2!=''" class="ml5 iconfont iconshexiangtou "></i> <el-button @click="handleChooseCompany(3)" type="primary" class="pa" size="mini">添加摄像头</el-button>
       <div>
         <video-player class="vjs-custom-skin" 
                        :options="playerOptions2" 
                        @ready="playerReadied">
          </video-player>
             <div class="video-text"><p class="mb5"><span class="tr">温度：</span><span class="tl">{{sensor_log3.temp}}℃</span></p><p class="mb5"><span class="tr">湿度：</span><span class="tl">{{sensor_log3.humidity}}</span></p><p class="mb5"><span class="tr">烟雾：</span><span class="tl">{{sensor_log3.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">甲烷：</span><span class="tl">{{sensor_log3.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">液化气：</span><span class="tl">{{sensor_log3.mq5_monitor_range}}</span></p><p class="mb5"><span class="tr">一氧化碳：</span><span class="tl">{{sensor_log3.mq9_monitor_range}}</span></p><p class="mb5"><span class="tr">水质：</span><span class="tl">{{sensor_log3.tds}}</span></p><p class="mb5"><span class="tr">酸碱度：</span><span class="tl">{{sensor_log3.ph}}</span></p></div>
          </div></div></el-col>
       <el-col :span="12" class=""><div class="col pr" ref="Purple3" :style="{width:fullWidth/2 + 'px',height:fullWidth/2*0.50 + 'px'}"><span @click="godetail1(play_video_name3.company_area_code)">{{play_video_name3.company_area_name}}</span><i @click="godetail1(play_video_name3.company_area_code)" v-if="play_video_name3!=''" class="ml5 iconfont iconshexiangtou "></i> <el-button @click="handleChooseCompany(4)" type="primary" class="pa" size="mini">添加摄像头</el-button>
          <div>
            <video-player class="vjs-custom-skin" 
                        :options="playerOptions3" 
                        @ready="playerReadied">
          </video-player>
            <div class="video-text"><p class="mb5"><span class="tr">温度：</span><span class="tl">{{sensor_log4.temp}}℃</span></p><p class="mb5"><span class="tr">湿度：</span><span class="tl">{{sensor_log4.humidity}}</span></p><p class="mb5"><span class="tr">烟雾：</span><span class="tl">{{sensor_log4.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">甲烷：</span><span class="tl">{{sensor_log4.mq2_monitor_range}}</span></p><p class="mb5"><span class="tr">液化气：</span><span class="tl">{{sensor_log4.mq5_monitor_range}}</span></p><p class="mb5"><span class="tr">一氧化碳：</span><span class="tl">{{sensor_log4.mq9_monitor_range}}</span></p><p class="mb5"><span class="tr">水质：</span><span class="tl">{{sensor_log4.tds}}</span></p><p class="mb5"><span class="tr">酸碱度：</span><span class="tl">{{sensor_log4.ph}}</span></p></div></div></div></el-col>
  ...
  >
     <el-form :inline="true" ref="addForm"  :model="companyPage" class="demo-form-inline">
      <el-form-item prop="company_area_name" size="">
          <el-input placeholder="小作坊区域名称" v-model="companyPage.company_area_name" class="input-with-select"  clearable @clear="onSearch" >
...
export default {
  ...
  data () {
    return {
      ...
      sensor_log3:'',
      sensor_log4:'',
      fullWidth:document.documentElement.clientWidth-100,
  ...
  mounted(){
      ...
      // console.log(this.$refs.Purple.style.height)
  ...
  methods: {
   ...
   getDetail(code){
      axios.get("/api/device/area", {company_area_code:code}).then(response => {
       ...
       axios.get('/api/sensor/area',{company_area_code:code}).then((v) => {
         if(JSON.stringify(v)!='{}'){this.sensor_log1=v.sensor.sensor_log;}else{this.sensor_log1='';}
          })
        }else if(this.type==2){
       ...
       axios.get('/api/sensor/area',{company_area_code:code}).then((v) => {
         if(JSON.stringify(v)!='{}'){this.sensor_log2=v.sensor.sensor_log;}else{this.sensor_log2='';}
          })
        }else if(this.type==3){
       ...
       axios.get('/api/sensor/area',{company_area_code:code}).then((v) => {
         if(JSON.stringify(v)!='{}'){this.sensor_log3=v.sensor.sensor_log;}else{this.sensor_log3='';}
          })
        }else if(this.type==4){
       ...
       axios.get('/api/sensor/area',{company_area_code:code}).then((v) => {
         if(JSON.stringify(v)!='{}'){this.sensor_log4=v.sensor.sensor_log;}else{this.sensor_log4='';}
          })
        }
          this.drawer = false;
      });
        this.getSensorArea(code)
   },
    godetail1(val){
      ...
      axios.get("/api/device/area", { company_area_code: val }).then(response => {
        ...
        this.getSensorArea(val)
    },
    getSensorArea(params){
        axios.get('/api/sensor/area',{company_area_code:params}).then((v) => {
         if(JSON.stringify(v)!='{}'){
           this.list=v.sensor;
...
.shopList{
 ...
 background-size:contain;
     ...
     span{display: inline-block;margin-top: 3px;color: #000;font-weight: bolder;width: 100px;height: 20px;
    overflow: hidden;
 ...
 .video-text{position: absolute;right: 3%;top:18%;width: 20%;
 p{display: flex;}
 span{color: #7dbcfc !important;font-weight: 300 !important;font-size: 13px;}}
                /deep/ .video-player{
                  margin: 13px 36px 0px 36px;
                  .vjs-fluid{width: 80%;height: 80%;}
                .video-js{
