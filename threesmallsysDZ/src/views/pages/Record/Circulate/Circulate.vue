<template>
  <el-container class="shopList ">
    <el-header class="box add bb">
        <h5 ><span class="lines"></span>备案管理/食品流通</h5>
         <!-- v-if="user_level==5" -->
        <div><el-button v-if="menu_btns!=null&&menu_btns.indexOf('5-2-1')!== -1"  class=""  icon="el-icon-plus" size="small" type="primary" @click="$router.push({path: 'circulate/add'})">新增备案</el-button></div>
    </el-header>
  ...
  >
   <el-row class="box search">
       <el-form :inline="true"  :model="page" class="demo-form-inline">
        ...
        <el-col :span="4" class="mr10">
           <el-form-item size="small">
            <el-select v-model="page.department_code" clearable filterable placeholder="监管部门"  @change="change(3,page.department_code)">
    ...
    <div class="box list">
       <div v-if="page.filing_state=='3'" class="tl list_tab ">
          ...
          <el-button  :class="activeClass ==k?'active':''" size="small" @click="IsActive(k,v.id)">{{v.title}} <span v-if="v.id=='2'" class="list_tag pa disib bgcred cfff">{{count.expired}}</span><span v-if="v.id=='4'" class="list_tag pa disib bgc666 cfff">{{count.will_expire}}</span></el-button>  
          </div>
         </div>
      <el-table border  :data="tableData"  style="width: 100%">
        ...
        <el-table-column  min-width="120" :show-overflow-tooltip="true" label="营业面积"  align='center' :formatter="userTypeList"></el-table-column>
...
export default {
  data () {
    ...
    return {
      ...
      options2:'',
      activeClass:0,
      editableTabs: [{title: '已归档',name: '3'},{title: '已驳回',name: '4'},{title: '待审核',name: '1'},{title: '黑名单',name: '5'}],
       list_btn:[{id:'1001574645421581111111102',title: '四川省小经营店备案证（流通）'},{id:'1001574645421581111111105',title: '食品经营许可证（流通）'}],
   ...
   created () {
     var that=this;
      ...
      that.page = Object.assign({}, that.recordPage1);
  ...
  methods: {
    ...
    userTypeList3(row){
       if(row.companytag_code=='1001574645421581111111102'){return '四川省小经营店备案证（流通）'}else{return '食品经营许可证（流通）'}
    },
     handleClick() {
        var that=this;
    ...
    visibleChange(val){
      ...
      change(index,val){
        var that=this;
         if (val == null) val = '';
if(index==1){that.page.companytag_code=val;that.page.business_form='';this.options2=''}
 ...
 this.onSubmit()
    ...
    // 搜索
     onSubmit () { 
      ...
      that.$store.dispatch('record/setNewPage1', that.page)
      that.init(that.page)
    },
    // 分页
    currentChange(page){
  ...
  goUpdate(index){
    this.$router.push({
        ...
        query: {
          id: index,
          code:'1001574645421581000000002'
        }
      });
  },
...
    if(this.activeName==1){this.page.down_name = '食品流通待审核备案信息'}else if(this.activeName==5){this.page.down_name = '食品流通黑名单备案信息'}else if(this.activeName==3){this.page.down_name = '食品流通已归档' + statename + '备案信息'}else if(this.activeName==4){this.page.down_name = '食品流通已驳回备案信息'}
