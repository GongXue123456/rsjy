<template>
  <el-container class="shopList ">
    <el-header class="box add bb">
        <h5 ><span class="lines"></span>备案管理/食品生产</h5>
        <div><el-button v-if="menu_btns!=null&&menu_btns.indexOf('5-1-1')!== -1"  class=""  icon="el-icon-plus" size="small" type="primary" @click="$router.push({path: 'produce/add'})">新增备案</el-button></div>
    </el-header>
        ...
        <el-table-column  prop="business_type" min-width="100" :show-overflow-tooltip="true" align='center' label="业务类型">
         ...
         <el-table-column prop="state" label="证件状态" min-width="100" :show-overflow-tooltip="true" align='center'>
           <template slot-scope="scope">
              ...
              <span v-else-if="scope.row.state==4" style="color: #FAA500">即将过期</span>
        </template>
         </el-table-column>
...
export default {
  data () {
    return {
      ...
      list_btn:[{id:'1001574645421581111111101',title: '四川省小作坊备案证'}],
   ...
   created () {
     var that=this;
      ...
      that.page = Object.assign({}, that.recordPage);
  ...
  methods: {
    ...
    userTypeList3(row){
       if(row.companytag_code=='1001574645421581111111101'){return '四川省小作坊备案证'}
    },
     handleClick() {
        var that=this;
    ...
    // 搜索
     onSubmit () { 
      ...
      that.$store.dispatch('record/setNewPage', that.page)
      that.init(that.page)
    },
    // 分页
    currentChange(page){
...
  DownExcel () { 
    this.loading=true;
    this.timer=setInterval(() => { this.check("export_company_search");}, 2000);
    this.currentPage = 1
...
    if(this.page.search_time_bigen==null){this.page.search_time_bigen='';}
    ...
    if(this.activeName==1){this.page.down_name = '食品生产待审核备案信息'}else if(this.activeName==5){this.page.down_name = '食品生产黑名单备案信息'}else if(this.activeName==3){this.page.down_name = '食品生产已归档' + statename + '备案信息'}else if(this.activeName==4){this.page.down_name = '食品生产已驳回备案信息'}
    
