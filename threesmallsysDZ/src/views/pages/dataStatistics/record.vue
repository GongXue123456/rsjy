    ...
    <el-main class="pt0">
      ...
      <div class="echarts">
          ...
          <el-row class="mt20 c999 f14">
           <el-col :span="5">备案数量<p class="mt5"><span class="c333 f25 fb mr5">{{data1.company_all}}</span>家</p></el-col>
           <el-col :span="5">待审核<p class="mt5"><span class="c333 f25 fb mr5">{{data1.company_no_check}}</span>家</p></el-col>
           <el-col :span="5">已归档<p class="mt5"><span class="c333 f25 fb mr5">{{data1.company_pass_check}}</span>家</p></el-col>
           <el-col :span="5">已驳回<p class="mt5"><span class="c333 f25 fb mr5">{{data1.company_back_check}}</span>家</p></el-col>
           <el-col :span="5">审核率<p class="mt5"><span class="c333 f25 fb mr5">{{data1.company_examine_rate*100}}%</span></p></el-col>
          </el-row>
      ...
      <div class="box list  mt20">
         <div class="title f15">
          <p><span class="triangle-right fl ml10 mr10"></span>备案统计明细表</p>
        ...
        <el-table  class="mt20  clear" border :data="tableData" style="width: 100%" fit>
          ...
          <el-table-column prop="company_all" label="备案数" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    init(params) {
       ...
       axios.get('/api/statistics/company/putOnRecord', params).then((v) => {
           this.data1=v
      })
      axios.get('/api/statistics/company/putOnRecordList', params).then((v) => {
    ...
      DownExcel () { 
    this.loading=true;
    this.timer=setInterval(() => { this.check("export_putonrecordlist_search");}, 2000);
    if(this.page.search_time_end==null){this.page.search_time_end='';}
    if(this.page.search_time_bigen==null){this.page.search_time_bigen='';}
    let statename
    if(this.page.search_time_end!=''&&this.page.search_time_bigen!=''){
        statename = this.page.search_time_bigen+'至'+this.page.search_time_end+'备案统计明细表';
    }else{
        statename ='备案统计明细表';
    }
   this.page.down_name = statename
    axios.fileget('/api/report/export_putonrecordlist_search', this.page).then((v) => {})
  },
check(d_name){
...
.shopList {
  ...
  .echarts{
      .el-col-5{border: 1px solid #ccc;border-right: none;padding: 10px;width: 20%;}
      .el-col-5:last-child{border-right: 1px solid #ccc;}
  }
