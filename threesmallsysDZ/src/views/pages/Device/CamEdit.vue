<template>
  <el-container class="camedit">
    <el-header class="box add bb">
      <h5 class="mb10">
        <span class="lines"></span>编辑摄像头
      </h5>
      <div>
        <el-button class size="small" type="primary" @click="handleUpdate()">更新</el-button>
    ...
    <el-main class="pt0">
      <el-form
        ...
        :model="form"
      >
        <el-form-item label="设备类型：" prop="device_type" :rules="[{required: true,message: '请选择设备类型', trigger: ['blur','change']}]">
          <el-select
        ...
        <el-form-item label="阿里云企业ID：" v-if="form.device_type==1" prop="device_id" :rules="[{required: true,message: '请输入阿里云企业ID', trigger: 'blur' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入空格等特殊符号' }]">
          <el-input v-model="form.device_id" size="small" placeholder="请填入siteId"></el-input>
        </el-form-item>
        <el-form-item label="播放地址：" v-if="form.device_type==2" prop="play_video_url" :rules="[{required: true,message: '请输入播放地址', trigger: 'blur' }]">
          <el-input v-model="form.play_video_url" size="small"></el-input>
        </el-form-item>
...
export default {
  ...
  filters: {},
  mounted() {
    this.camId = this.$route.query.id;
    this.initForm(this.camId);
  },
  methods: {
    resetForm() {
      this.form = {
        device_code: this.camId,
        device_type: "",
        device_id: "",
        company_code: "",
        company_name: "",
    ...
    initForm(id) {
      axios.get("/api/device/single", { device_code: id }).then(response => {
        this.form = response.device;
         this.areaPage.company_code = response.device.company_code;
      });
    },
    handleTypeChange() {
    ...
    onClose() {
      this.$refs["editForm"].validateField("company_name");
       this.selected='';
      this.type=''
    },
    handleConfirm() {
    ...
    handleUpdate() {
      this.$refs["editForm"].validate(valid => {
        if (valid) {
          axios.put("/api/device/update", this.form).then(response => {
            this.$router.back();
          });
        } else {
          return false;
