<template>
  <el-container class="camadd">
    <el-header class="box add bb">
      <h5 class="mb10">
        <span class="lines"></span>编辑传感器预警项
      </h5>
      <div>
...
export default {
  ...
  filters: {},
  mounted() {
    this.sensor_threshold_code = this.$route.query.id;
    this.initForm(this.sensor_threshold_code);
  },
  methods: {
    handleUpdate(dataForm) {
        var data_form = this.dataForm;
        var _sensor_threshold_code = this.sensor_threshold_code;
        this.$refs[dataForm].validate((valid) => {
            if(valid){
                 axios.get("/api/sensorThreshold/exist", {sensor_threshold_code:this.sensor_threshold_code,sensor_enname:data_form.sensor_enname}).then(v => {
                    axios.put("/api/sensorThreshold/update",data_form).then(response => {
                      this.$router.back();
                    })
                 });
            }
    ...
    initForm(id) {
      axios.get("/api/sensorThreshold/single", { sensor_threshold_code: id }).then(response => {
        this.dataForm = response.sensor_threshold;
      });
    },
    exist(sensor_enname) {
       if(sensor_enname != null && sensor_enname.length > 0){
            axios.get("/api/sensorThreshold/exist", {sensor_threshold_code:this.sensor_threshold_code,sensor_enname:sensor_enname}).then(v => {
            });
       }
    }
  }
