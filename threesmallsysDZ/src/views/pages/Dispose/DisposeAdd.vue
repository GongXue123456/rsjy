<template>
  <el-container class="disposeadd">
    ...
    <el-main class="pt0">
      ...
      >
        <el-form-item label="处理方式名称：" prop="dispose_name" :rules="[{required: true,message: '请输入处理方式名称', trigger: 'blur' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入空格等特殊符号' }]">
          <el-input
            v-model="formInline1.dispose_name"
            size="small"
...
export default {
  ...
  mounted() {},
  created () {
    this.dispose_code=this.$route.query.id;
    this.formInline1.dispose_code=this.$route.query.id;
  ...
  methods: {
    init (params) {
      if(this.id1==1){
        axios.get('/api/dispose/single', {dispose_code:this.dispose_code}).then((v) => {
          this.formInline1=v.patrolDispose
        })
      }
    },
    submitForm(formInline1){
      ...
      this.$refs[formInline1].validate((valid) => {
        if (valid) {
          if(this.id1==1){              
            axios.put('/api/dispose/update', paramas).then((v) => {
              this.$router.back()
            })
          }else{
            axios.post('/api/dispose/save', paramas).then((v) => {
              this.$router.back()
            })
          }
