<template>
  <el-container class="patroladd">
    <el-header class="box add bb">
        <h5 class="mb10"><span class="lines"></span>新闻编辑</h5>
        <div>
    ...
    <el-main class="p15 pt0">
        <el-form :inline="false"  status-icon ref="formInline"  :model="formInline"  class="demo-ruleForm">
            ...
            <el-row class="box mt20  f14">
                <el-col :span="22" class="tl" :offset="1">
                  <el-form-item class="w" label-width="100px" label="内容：" size="small"  >
                   <Editor v-model="formInline.news_content" :list.sync="content"></Editor>
                  </el-form-item>
                </el-col>
            </el-row>
            <el-row class="box mt20  f14">
            ...
            <el-row class="box mt20  f14">
                <el-col :span="22" class="tl" :offset="1">
                  <el-form-item class="w" label-width="100px" label="新闻封面：" size="small" >
                    <el-upload
...
export default {
  ...
  created () {
    ...
    let code = this.$route.query.news_code
    this.formInline.news_code = code
    this.formInline.user_code = this.user.user_code
    this.init(code)
  },
  methods: {
    init (news_code) {
      axios.get("/api/news/single", {news_code: news_code}).then(v => {
        this.formInline.news_title = v.news_title
        this.formInline.news_state = v.news_state + ''
        this.formInline.news_summary = v.news_summary
        this.formInline.news_content = v.news_content
        this.content = v.news_content
        this.formInline.news_category_code = v.news_category_code
        this.formInline.companytype_codes = v.companytype_codes.split(",")
        if(v.news_imgurl){
          this.imageUrl = this.user.qyurl + v.news_imgurl
        }
        this.formInline.news_imgurl = v.news_imgurl
        if(v.news_province!=null){this.formInline.news_province=v.news_province;}
        if(v.news_city!=null){this.formInline.news_city=v.news_city;}
        if(v.news_area!=null){this.formInline.news_area=v.news_area;}
        if(v.news_town!=null){this.formInline.news_town=v.news_town;}
        if(v.news_vill!=null){this.formInline.news_vill=v.news_vill;}
        this.getProvince();
      })
    ...
    change(index,val){
        ...
        this.options3.forEach(item=>{
         ...
         if(index==5){
            ...
            if(item.region_code==val){
              mythis.formInline.news_vill=item.region_name
            }
        })
        }
     },
     submitForm(formInline){
        ...
        if (valid) {
          params.companytype_codes = params.companytype_codes.join(",")
          axios.put('/api/news/update', params).then((v) => {
            this.$router.back()
         })
        }
      })
