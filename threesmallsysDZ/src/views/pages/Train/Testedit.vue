<template>
  <el-container class="patroladd">
    ...
    <el-header class="box add bb">
        <h5 class="mb10"><span class="lines"></span>编辑试卷</h5>
        <div>
    ...
    <el-main class="pt0">
     ...
     <el-row class="box mt20  f14">
        ...
        <el-form-item  class="rules w" label-width="10%" label="试卷标题："  size="small"  prop="train_exam_title" :rules="[{required: true,message: '请输入课程标题', trigger: 'blur' }]">
         ...
         <el-col :span="22"  class="tl" :offset="1">
          <el-form-item class="rules w" label-width="10%" label="合格题数：" size="small" >
             <el-input  style="width:100%" placeholder="合格题数" v-model="formInline.train_exam_standard" class="input-with-select">
            </el-input>
        </el-form-item>
        </el-col>
     </el-row>
     <el-row class="box mt20  f14">
        ...
        <el-col :span="22" class="tl" :offset="1" >
            <el-form-item class="rules w"  size="small"  label-width="10%" :label="'试题'+(index+1)+'：'" :prop="'train_exam_topic_list.' + index +'.train_exam_topic_title'" :rules="Rules.Name1">
        ...
        <el-col :span="9"  class="tl" :offset="1">
          <el-form-item label="正确答案："   class="w" label-width="24%" size="small" :prop="'train_exam_topic_list.' + index +'.train_exam_topic_standard'" :rules="Rules.Name6">
            <el-select class="w" v-model="item.train_exam_topic_standard" placeholder="请选择区县" >
...
export default {
    ...
    return {
     ...
     created () {
      this.user = local.get('user');
         this.formInline.train_exam_code=this.$route.query.id;
  ...
  methods: {
      init (params) {
          axios.get('/api/trainExam/single',{train_exam_code:this.formInline.train_exam_code}).then((v) => {
      this.formInline.train_exam_code=v.train_exam_code
            this.formInline.train_exam_title=v.train_exam_title
            this.formInline.train_exam_standard=v.train_exam_title

            v.train_exam_topic_list.forEach(item=>{
              this.formInline.train_exam_topic_list.push(objKeySort({
                 train_exam_code:item.train_exam_code,
           train_exam_topic_code:item.train_exam_topic_code,
            train_exam_topic_option_a:item.train_exam_topic_option_a,
            train_exam_topic_option_b:item.train_exam_topic_option_b,
            train_exam_topic_option_c:item.train_exam_topic_option_c,
            train_exam_topic_option_d:item.train_exam_topic_option_d,
            train_exam_topic_standard:item.train_exam_topic_standard,
           train_exam_topic_title:item.train_exam_topic_title,
              }))
            })
       Vue.delete(this.formInline,'sign');
...
deleteRules(item, index) {
     ...
     submitForm(formInline){
        ...
        if (valid) {
axios.put('/api/trainExam/update', paramas).then((v) => {
           this.$router.back()
          }) 
        }
         })
