<template>
  <el-container class="camedit">
    <el-header class="box add bb">
      <h5 class="mb10">
        <span class="lines"></span>编辑传感器
      </h5>
      <div>
        <el-button class size="small" type="primary" @click="handleUpdate('dataForm')">更新</el-button>
    ...
    <el-main class="pt0">
      ...
      >
        <el-form-item label="传感器名称：" prop='sensor_name'  :rules="[{required: true,message: '请输入传感器名称', trigger: 'blur' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入空格等特殊符号' }]">
          <el-input
        ...
        <el-form-item label="传感器编号："  prop='sensor_number'  :rules="[{required: true,message: '请输入传感器编号', trigger: 'blur' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入空格等特殊符号' }]">
          <el-input
            v-model="dataForm.sensor_number"
            size="small"
            clearable
            @blur="exist(dataForm.sensor_number)"
          ></el-input>
        </el-form-item>
        <el-form-item label="传感器描述：" prop='sensor_desc'  :rules="[{required: true,message: '请输入传感器描述', trigger: 'blur' },{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+$/, message: '不允许输入空格等特殊符号' }]">
          <el-input
            type="textarea"
        ...
        <el-form-item label="绑定企业：" prop='company_name' :rules="[{required: true,message: '请绑定企业',trigger: ['blur','change']}]">
          <el-input v-model="dataForm.company_name" size="small" disabled>
            <el-button slot="append" icon="el-icon-search" @click="handleChooseCompany(1)">请选择企业</el-button>
          </el-input>
        </el-form-item>
        <el-form-item label="绑定企业区域：" prop="company_area_name" :rules="[{required: true,message: '请绑定企业区域',trigger: ['blur','change']}]">
          <el-input v-model="dataForm.company_area_name" size="small" disabled>
            <el-button slot="append" icon="el-icon-search" @click="handleChooseCompany(2)">请选择企业区域</el-button>
          </el-input>
        </el-form-item>
        <!-- <el-form-item label="图标：" prop='sensor_logo_url'>
            <el-image  v-if="dataForm.sensor_logo_url"
      :src="user.qyurl+dataForm.sensor_logo_url"
      :preview-src-list="[user.qyurl+dataForm.sensor_logo_url]"
      ></el-image>
       <el-image v-else
      :src="require('../../../assets/image/moren.png')"
      ></el-image>
        </el-form-item> -->
        <el-form-item   label="上传图片：" size="small">
      ...
      <el-dialog :title="type==1?'企业列表':'企业区域列表'" :visible.sync="dialogTableVisible" @close="onClose">
        <el-row class="box search mt20" :gutter="10">
          <el-form  v-if="type==1"  :inline="true" :model="companyPage" class="demo-form-inline">
            ...
            <el-col :span="6">
              <el-form-item size="small">
                <el-select v-model="companyPage.companytag_code"  placeholder="请选择主体类型" clearable @change="onSearch(companyPage.companytag_code)">
                  <el-option
        ...
        <el-table :data="companyData" border fit v-loading="companyListLoading">
          <el-table-column label="选择" width="90" header-align="center" align="center">
          ...
          <el-table-column property="company_name" label="企业名称" align="center" min-width="150px"></el-table-column>
          <el-table-column v-if="type==1" label="食品监管分类" align="center">
            <template slot-scope="scope">{{scope.row.companytype_name}}</template>
          </el-table-column>
          <el-table-column v-if="type==1" label="企业主体类型" align="center">
          ...
          <el-table-column v-if="type==1"  label="地址" align="center" min-width="150" :show-overflow-tooltip="true">
           ...
           <el-table-column v-else-if="type==2"  property="company_area_name" label="企业区域名称" align="center" min-width="150px"></el-table-column>
        </el-table>
...
export default {
  ...
  mounted() {
    ...
    this.sensor_code = this.$route.query.id;
    this.initForm(this.sensor_code);
  },
  methods: {
    resetForm() {
      this.dataForm = {
    ...
    initForm(id) {
      axios.get("/api/sensor/single", { sensor_code: id }).then(response => {
        // this.dataForm = response.sensor;
        this.dataForm.sensor_code = response.sensor.sensor_code;
        this.dataForm.company_code = response.sensor.company_code;
         this.dataForm.company_name = response.sensor.company_name;
         this.dataForm.company_area_name = response.sensor.company_area_name;
         this.dataForm.company_area_code = response.sensor.company_area_code;
         this.dataForm.sensor_name = response.sensor.sensor_name;
         this.dataForm.sensor_number = response.sensor.sensor_number;
         this.dataForm.sensor_desc = response.sensor.sensor_desc;
         this.dataForm.sensor_logo_url = response.sensor.sensor_logo_url;
         this.areaPage.company_code = response.sensor.company_code;
        if(response.sensor.sensor_logo_url!=''&&response.sensor.sensor_logo_url!=null){
          this.uploadDisabled=true;
          this.fileList=[{name: '', url: this.user.qyurl+response.sensor.sensor_logo_url}]
        }
      });
    },
    ...
    handleUpdate(dataForm) {
      var data_form = this.dataForm;
        this.$refs[dataForm].validate((valid) => {
            if(valid){
                  axios.get("/api/sensor/exist", {sensor_code:this.sensor_code,sensor_number:data_form.sensor_number}).then(v => {
                     axios.put("/api/sensor/update", data_form).then(response => {
                      this.$router.back();
                    })
                 });
            }
    ...
    exist(sensor_number) {
       if(sensor_number != null && sensor_number.length > 0){
          axios.get("/api/sensor/exist", {sensor_code:this.sensor_code,sensor_number:sensor_number}).then(v => {
          });
       }
    }
  }
